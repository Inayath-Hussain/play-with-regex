...
const FlagsInput: React.FC<Iprops> = ({ value, name, selected, handleSelection }) => {

    const newName = name.replace(value, `<em>${value}</em>`)

    const sanitizedText = sanitizeHTML(newName, { allowedTags: ['em'] })


    return (
        <>
            <label htmlFor={value} className="flex flex-row justify-center items-center gap-1 cursor-pointer w-max">
                <input type="checkbox" name="" id={value} tabIndex={50} className="w-0 h-0" onChange={() => handleSelection(value)} />
